...
#include <iostream>
/*
 * The KVStore class provides a robust key-value store implementation using a combination of an in-memory SkipList and persistent SSTable files, supporting efficient get, put, delete, and reset operations. It also includes a compaction process to manage storage efficiently.
 */

/*
 * The constructor initializes the KVStore instance, setting up the memTable and loading existing SSTables from the specified directory.
 */
...
/*
 * The destructor ensures that any remaining entries in the memTable are saved to an SSTable and performs compaction. It also cleans up allocated memory.
 */
KVStore::~KVStore()
{
 ...
 * Inserts or updates a key-value pair in the memTable. If the memTable exceeds a certain size, it is saved to an SSTable.
...
/*
 * These methods handle the compaction process, merging SSTables to reduce redundancy and optimize storage.
 */
